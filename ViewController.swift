...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    // MARK: - Collect View Delegate Method
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        // Check if there's any time remaining. Don't let the user interact if the is 0
        if milliseconds <= 0 {
            return
        }
    ...
    func checkForMatch(_ secondFlippedCradIndex:IndexPath) {
        
        // Get the two card objects for the two indices and see if they match
        let cardOne = cardsArray[firstFlippedCardIndex!.row]
        let cardTwo = cardsArray[secondFlippedCradIndex.row]
        
        // Get the two collection view cells that represent card one and card two
        let cardOneCell = collectionView.cellForItem(at: firstFlippedCardIndex!) as? CardCollectionViewCell
        let cardTwoCell = collectionView.cellForItem(at: secondFlippedCradIndex) as? CardCollectionViewCell
    ...
    func checkForGameEnd() {
        ...
        for card in cardsArray {
            ...
            if card.isMatched == false {
                ...
                // We've found a card is unmatched
                hasWon = false
                break
